...
/*
 * 
 * This file is part of Android under package com.android.server
 */

package com.novoda.lib.httpservice.server;
...
public class IntentResolver<F extends IntentFilter, R extends Object> {
    ...
    public void addFilter(F f) {
        ...
        if (numT != 0) {
            register_intent_filter(f, f.actionsIterator(), mTypedActionToFilter,
                    "      TypedAction: ");
        }
    }
    ...
    void removeFilterInternal(F f) {
        ...
        int numS = unregister_intent_filter(f, f.schemesIterator(), mSchemeToFilter,
                "      Scheme: ");
        ...
        if (numT != 0) {
            unregister_intent_filter(f, f.actionsIterator(), mTypedActionToFilter,
                    "      TypedAction: ");
        }
    }

    boolean dumpMap(PrintWriter out, String titlePrefix, String title, String prefix,
            Map<String, ArrayList<F>> map, String packageName, boolean printFilter) {
    ...
    public boolean dump(PrintWriter out, String title, String prefix, String packageName,
        ...
        if (dumpMap(out, curPrefix, "Full MIME Types:", innerPrefix, mTypeToFilter, packageName,
                printFilter)) {
            curPrefix = sepPrefix;
        }
        if (dumpMap(out, curPrefix, "Base MIME Types:", innerPrefix, mBaseTypeToFilter,
                packageName, printFilter)) {
            curPrefix = sepPrefix;
        }
        if (dumpMap(out, curPrefix, "Wild MIME Types:", innerPrefix, mWildTypeToFilter,
                packageName, printFilter)) {
            curPrefix = sepPrefix;
        }
        if (dumpMap(out, curPrefix, "Schemes:", innerPrefix, mSchemeToFilter, packageName,
                printFilter)) {
            curPrefix = sepPrefix;
        }
        if (dumpMap(out, curPrefix, "Non-Data Actions:", innerPrefix, mActionToFilter, packageName,
                printFilter)) {
            curPrefix = sepPrefix;
        }
        if (dumpMap(out, curPrefix, "MIME Typed Actions:", innerPrefix, mTypedActionToFilter,
                packageName, printFilter)) {
            curPrefix = sepPrefix;
        }
    ...
    public List<R> queryIntentFromList(Intent intent, String resolvedType, boolean defaultOnly,
        ...
        int N = listCut.size();
        for (int i = 0; i < N; ++i) {
            buildResolveList(intent, debug, defaultOnly, resolvedType, scheme, listCut.get(i),
                    resultList);
        }
    ...
    public List<R> queryIntent(Intent intent, String resolvedType, boolean defaultOnly) {
        ...
        if (debug)
            Log.v(TAG, "Resolving type " + resolvedType + " scheme " + scheme + " of intent "
                    + intent);
            ...
            if (slashpos > 0) {
                ...
                if (!baseType.equals("*")) {
                    if (resolvedType.length() != slashpos + 2
                        ...
                        if (debug)
                            Log.v(TAG, "First type cut: " + firstTypeCut);
                        secondTypeCut = mWildTypeToFilter.get(baseType);
                        if (debug)
                            Log.v(TAG, "Second type cut: " + secondTypeCut);
                    } else {
                    ...
                    if (debug)
                        Log.v(TAG, "Third type cut: " + thirdTypeCut);
                    ...
                    if (debug)
                        Log.v(TAG, "Typed Action list: " + firstTypeCut);
                }
            }
        }
        ...
        if (scheme != null) {
            ...
            if (debug)
                Log.v(TAG, "Scheme list: " + schemeCut);
        }
        ...
        if (resolvedType == null && scheme == null && intent.getAction() != null) {
            ...
            if (debug)
                Log.v(TAG, "Action list: " + firstTypeCut);
        }

        if (firstTypeCut != null) {
            buildResolveList(intent, debug, defaultOnly, resolvedType, scheme, firstTypeCut,
                    finalList);
        }
        if (secondTypeCut != null) {
            buildResolveList(intent, debug, defaultOnly, resolvedType, scheme, secondTypeCut,
                    finalList);
        }
        if (thirdTypeCut != null) {
            buildResolveList(intent, debug, defaultOnly, resolvedType, scheme, thirdTypeCut,
                    finalList);
        }
        if (schemeCut != null) {
    ...
    private void buildResolveList(Intent intent, boolean debug, boolean defaultOnly,
        ...
        for (i = 0; i < N; i++) {
            ...
            match = filter.match(intent.getAction(), resolvedType, scheme, intent.getData(),
                    categories, TAG);
            if (match >= 0) {
                if (debug)
     ...
     * "image/*", or "{@literal *}/*".
     */
    private final HashMap<String, ArrayList<F>> mTypeToFilter = new HashMap<String, ArrayList<F>>();
     ...
     */
    private final HashMap<String, ArrayList<F>> mBaseTypeToFilter = new HashMap<String, ArrayList<F>>();
     ...
     */
    private final HashMap<String, ArrayList<F>> mWildTypeToFilter = new HashMap<String, ArrayList<F>>();

    /**
     * All of the URI schemes (such as http) that have been registered.
     */
    private final HashMap<String, ArrayList<F>> mSchemeToFilter = new HashMap<String, ArrayList<F>>();

    /**
     * All of the actions that have been registered, but only those that did not
     * specify data.
     */
    private final HashMap<String, ArrayList<F>> mActionToFilter = new HashMap<String, ArrayList<F>>();

    /**
     * All of the actions that have been registered and specified a MIME type.
     */
    private final HashMap<String, ArrayList<F>> mTypedActionToFilter = new HashMap<String, ArrayList<F>>();
